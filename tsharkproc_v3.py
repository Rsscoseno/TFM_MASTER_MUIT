...
def ainflux(flow_list):
	...
	json_body =[]	
	#json_body.clear()
	#for flujo in range(len(flow_list)):
		#print("")
		#print("")
		#print ("La tabla de flujos que estoy comparando es",flow_list[flujo])
		#print("")
		#print("")#,json_body[item]["data"]["1IP_src"] )
		
	#	json_body.insert(flujo,{

	#		"measurement": "Medidas",
	#		"tags": {
	#			"flowid":str(flujo),
	#			"6Host":flow_list[flujo][4]},
	#		"fields":{
	#			"1IP_src":flow_list[flujo][0],
	#			"2IP_dst":flow_list[flujo][1],
	#			"3Port_src":flow_list[flujo][2],
	#			"4Port_dst":flow_list[flujo][3],
				#"5Protocol":flow_list[flujo][4],
				#"6Host":flow_list[flujo][5],
	#			"5#Bytes":int(flow_list[flujo][5]),
	#			"7#Paquetes":flow_list[flujo][6]}
	#		})
...
client.drop_database('tshark_flows')
client.create_database('tshark_flows')
client.get_list_database()

client.switch_database('tshark_flows')
cont_g = 0 
...
with codecs.open('salidatshark.csv','r', encoding='utf-8', errors='replace') as File:    
				...
				if row[6] == 'none': # Si me viene con el campo vacío, pero ya sabemos que host se lo ponemos
					...
					for j in range(len(flujos)): # Si no viene con campo none, si no con HOST se lo ponemos
						...
						if row[2:6] == flujos[j][0:4] or tupla_inversa == flujos[j][0:4]:
							...
							flujos[j][4] = row[6]
							#print("LLegas al punto de influx",flujos[j])
							ainflux(flujos[j])
							break
